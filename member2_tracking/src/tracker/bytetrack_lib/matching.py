...
def bbox_ious(bboxes1, bboxes2):
    ...
    xx1 = np.maximum(bboxes1[:, 0][:, None], bboxes2[:, 0])
    yy1 = np.maximum(bboxes1[:, 1][:, None], bboxes2[:, 1])
    xx2 = np.minimum(bboxes1[:, 2][:, None], bboxes2[:, 2])
    yy2 = np.minimum(bboxes1[:, 3][:, None], bboxes2[:, 3])
...
def gate_cost_matrix(kf, cost_matrix, tracks, detections, only_position=False):
    ...
    # Ora kalman_filter è importato, quindi questa riga funzionerà
