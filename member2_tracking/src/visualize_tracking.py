...
def load_json_data(json_path):
    ...
    print(f"Loaded {count_objs} tracked objects across {len(tracker_map)} frames.")
    return tracker_map
...
def get_frame_generator(source):
    if os.path.isfile(source):
        ...
        images.sort() 
        if not images: raise ValueError("Image folder is empty")
        ...
        raise ValueError("Invalid input source (must be a file or a directory).")

def run_visualizer():
    parser = argparse.ArgumentParser(description="Visualize Tracking Results")
    
    parser.add_argument('--video', type=str, required=True, help="Path to input video file")
    parser.add_argument('--json', type=str, required=True, help="Path to tracking output JSON")
    parser.add_argument('--output', type=str, default="output_video.mp4", help="Path for output video file")
    
    args = parser.parse_args()

    try:
        tracker_data = load_json_data(args.json)
    except Exception as e:
        print(f"Data Error: {e}")
        return

    try:
        frame_gen, fps, size, total_frames = get_frame_generator(args.video)
    except Exception as e:
    ...
    out.release()
    print(f"Video visualization completed. Saved to: {os.path.abspath(args.output)}")
